    ...
    <section id="digital-avatar" className="py-16 md:py-24 bg-gradient-to-b from-tech-dark to-tech-dark/90 border-t border-[#0EA5E9]/10">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-10 animate-fade-up">
          ...
          <div className="mt-8 flex justify-center">
            <Button
              ...
              className="bg-gradient-to-r from-[#0EA5E9] to-[#9333EA] text-white font-medium rounded-xl px-8 py-6 text-lg shadow-neon-purple hover-float group hover:scale-105 transition-all duration-300"
              size="lg"
            >
              <Bot className="h-6 w-6 mr-3" />
              <span>Start Chatting with My Digital Twin</span>
              <ArrowUpRight className="ml-3 h-5 w-5 group-hover:translate-x-1 transition-transform" />
            </Button>
          </div>
        </div>
        ...
        <div className="max-w-5xl mx-auto mt-12">
          <div className="glass-card bg-black/20 border border-[#0EA5E9]/20 rounded-2xl p-6 shadow-neon-cyan">
            <div className="flex justify-between items-center mb-6">
              <h4 className="text-xl font-semibold text-white">Specialized Topics</h4>
              <Button 
                variant="outline" 
                onClick={() => setIsExpanded(!isExpanded)}
                className="border-[#0EA5E9]/30 text-white bg-[#0EA5E9]/10 hover:bg-[#0EA5E9]/20 shadow-neon-cyan"
              >
            ...
            {/* Grid display for popular domains with improved design */}
            <div className={`grid grid-cols-1 md:grid-cols-3 gap-4 ${isExpanded ? '' : 'md:grid-rows-1 overflow-hidden'}`}>
              {domains.slice(0, isExpanded ? domains.length : 3).map((domain, index) => {
                ...
                return (
                  <motion.div
                    key={domain.id}
                    className="glass-card p-4 rounded-xl border border-white/10 hover:border-[#0EA5E9]/50 transition-all cursor-pointer bg-black/30 hover:bg-black/40 shadow-neon-cyan hover:shadow-lg hover:scale-105"
                  ...
                  >
                    <div className="flex items-center mb-3">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white mr-3 shadow-neon-cyan`} style={{ backgroundColor: domain.color }}>
                        <Icon size={18} />
                      </div>
                    ...
                    <Button variant="default" size="sm" className="w-full justify-between mt-2 bg-gradient-to-r from-[#0EA5E9]/20 to-[#9333EA]/20 text-white hover:from-[#0EA5E9]/30 hover:to-[#9333EA]/30 border border-[#0EA5E9]/30">
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="w-full max-w-4xl h-[80vh] p-0 bg-black/90 border-[#0EA5E9]/30 shadow-neon-cyan">
          <DialogHeader className="p-6 border-b border-[#0EA5E9]/20 flex-row justify-between items-start bg-gradient-to-r from-[#0EA5E9]/10 to-[#9333EA]/10">
            <div>
              <DialogTitle className="text-xl mb-2 text-white">{domain?.title || 'Digital Avatar'}</DialogTitle>
